      ...
      <el-card>
        <div class="panel-search__results-group-inner">
          <d2-panel-search-item
            ...
            :item="item"
            :hover-mode="true"
            @click.native="handleResultsGroupItemClick(item.fullPath, item)"/>
        </div>
      </el-card>
    </div>
  </div>
...
export default {
  ...
  methods: {
     ...
     */
    handleResultsGroupItemClick (path, indexPath) {
      ...
      // 用户选择的是其它页面
      this.handleMenuSelect(path, indexPath)
    },
    /**
