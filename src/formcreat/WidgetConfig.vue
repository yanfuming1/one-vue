    ...
    <div v-if="show">
        <el-form label-position="top" size="small">
            ...
            <el-form-item label="标签宽度" v-if="Object.keys(data.options).indexOf('labelWidth')>=0 && !data.options.tableColumn">
                ...
                <el-input-number v-model="data.options.labelWidth" :disabled="!data.options.isLabelWidth" :min="0" :max="200" :step="10"></el-input-number>
            </el-form-item>
          
            <el-form-item label="复合型输入框" v-if="data.type == 'input'">
                <el-row>
                    <el-col :span="12">
                        <el-checkbox v-model="data.options.frontSlot">前置元素</el-checkbox>
                    </el-col>
                    <el-col :span="12">
                        <el-input v-model="data.options.frontText"  :disabled="!data.options.frontSlot"></el-input>
                    </el-col>
                </el-row>
                <el-row class="d2-mt-5">
                    <el-col :span="12">
                        <el-checkbox v-model="data.options.laterSlot">后置元素</el-checkbox>
                    </el-col>
                    <el-col :span="12">
                        <el-input v-model="data.options.laterText"  :disabled="!data.options.laterSlot"></el-input>
                    </el-col>
                </el-row>
            </el-form-item>

            <el-form-item label="复合型下拉框" v-if="data.type === 'select'">
                <el-col :span="6">
                    <span>icon:</span>
                </el-col>
                <el-col :span="18">
                    <el-input v-model="data.options.btnIcon"></el-input>
                </el-col>
            </el-form-item>

            <el-form-item label="后置按钮图标" v-if="data.type === 'input'">
                <el-col :span="6">
                    <span>icon:</span>
                </el-col>
                <el-col :span="18">
            ...
            <el-form-item label="是否可创建条目" v-if="data.type=='select'">
                <el-switch v-model="data.options.allowCreate" @change="data.options.allowCreate ? data.options.filterable = true : data.options.filterable = false"></el-switch>
            </el-form-item>
            <el-form-item label="是否多选" v-if="data.type=='cascader'">
                <el-switch v-model="data.options.multiple"></el-switch>
            </el-form-item>
            <el-form-item label="父子是否关联" v-if="data.type=='cascader'">
                <el-switch v-model="data.options.checkStrictly"></el-switch>
            </el-form-item>
            <el-form-item label="是否折叠" v-if="data.type=='cascader'">
                <el-switch v-model="data.options.collapseTags"></el-switch>
            </el-form-item>
            ...
            <el-form-item label="默认值" v-if="Object.keys(data.options).indexOf('defaultValue')>=0 && (data.type == 'text' || data.type == 'textarea' || data.type == 'input' || data.type=='number' || data.type=='rate' || data.type=='color' || data.type=='switch')">
                <el-input v-if="data.type=='textarea'" type="textarea" :rows="2" v-model="data.options.defaultValue"></el-input>
                ...
                <template v-if="data.type=='input' || data.type == 'text'">
                    <template v-if="data.options.dataType == 'number' || data.options.dataType == 'integer' || data.options.dataType == 'float'">
                        <el-input :type="data.options.dataType" v-model.number="data.options.defaultValue"></el-input>
                    </template>
                    <template v-else>
            ...
            <template v-if="data.type == 'time' || data.type == 'date'">
                ...
                <el-form-item label="限制时间范围" v-if="data.type == 'date'">
                    <el-select v-model="data.options.isDisabledDate">
                        <el-option value="1" label="当前时间之前可选择"></el-option>
                        <el-option value="2" label="当前时间之后可选择"></el-option>
                    </el-select>
                </el-form-item>
            ...
            <el-form-item label="操作属性">
                <el-checkbox v-model="data.options.hidden" v-if="Object.keys(data.options).indexOf('hidden')>=0 && !data.options.tableColumn">隐藏	</el-checkbox>
                ...
                <el-checkbox v-model="data.options.showPassword" v-if="Object.keys(data.options).indexOf('showPassword')>=0">显示密码</el-checkbox>
                <el-checkbox v-model="data.options.showScore" v-if="Object.keys(data.options).indexOf('showScore')>=0">显示分数</el-checkbox>
                <el-checkbox v-model="data.options.isShowSummary" v-if="Object.keys(data.options).indexOf('isShowSummary')>=0">启用合计</el-checkbox>
                <el-checkbox v-model="data.options.isOpenSummary" v-if="Object.keys(data.options).indexOf('isOpenSummary')>=0 && data.options.tableColumn">字段是否合计</el-checkbox>
            </el-form-item>
            ...
            <template v-if="data.type != 'grid' && data.type != 'tabs'">
                ...
                <el-form-item label="校验">
                    ...
                    <el-select v-if="Object.keys(data.options).indexOf('pattern')>=0" v-model="data.options.pattern" size="mini" >
                        <el-option value="/^(0?(13[0-9]|15[012356789]|17[013678]|18[0-9]|14[57][0-9]{8})|([0-9]{4}|[0-9]{3})(-| ))?([0-9]{7,8})?$/" label="手机固话"></el-option>
                        <!-- <el-option value="/^((13[0-9])|(14[5,7,9])|(15([0-3]|[5-9]))|(166)|(17[0,1,3,5,6,7,8])|(18[0-9])|(19[8|9]))\\d{8}$/" label="手机"></el-option> -->
                        <el-option value="/^[1-9]\d{7}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}$|^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}([0-9]|X|x)$/" label="身份证"></el-option>
                        <el-option value="/^\w+$/" label="不能输入中文"></el-option>
                        <el-option value="/^[\u4E00-\u9FA5]/g" label="只能输入中文"></el-option>
                        <el-option value="/^[0-9]*[0-9][0-9]*$/" label="字符串数字"></el-option>
                        <el-option value="/^[0-9A-Z-]{12,20}$/" label="银行账号"></el-option>
                        <el-option value="/^\d+(\.\d+)?$/" label="非负浮点数"></el-option>
                         <el-option value="/^[1-9]\d*$/" label="正整数"></el-option>
                        <el-option value="" label="其他"></el-option>
                    </el-select>
                    <div v-if="Object.keys(data.options).indexOf('pattern')>=0 && data.options.pattern === ''">
