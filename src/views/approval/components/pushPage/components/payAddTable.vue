<template>
    <div class="qqtaddTable">
      <el-form :model="totalLine" class="demo-form-inline  d2-mt-20" :class="{'d2-mt-20':isShowDetails}" label-width="100px">
        <el-row>
          <el-col :span="8">
            <el-form-item label="合计金额:" style="width:100%">
              <el-input :class="{'isShowDetails':isShowDetails}" disabled size="small" v-model="totalLine.totalMoney"  placeholder="合计金额">
          ...
          <el-col :span="8">
            <el-form-item label="大写:" style="width:100%">
              <el-input :class="{'isShowDetails':isShowDetails}" disabled  size="small" v-model="totalLine.totalmoneyUpper"  placeholder="大写"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
      <el-form :rules="formData.rules" :model="formData"  ref="formDom" class="demo-ruleForm">
        <el-table
          ...
          <el-table-column prop="line1" label="户名">
              <template slot-scope="scope">
                <el-form-item :prop="'tableData.' + scope.$index + '.line1'" :rules='formData.rules.required'>
                  <el-select size="small" allow-create filterable default-first-option v-model="scope.row.line1" @visible-change="selectChange"  :class="{'isShowDetails':isShowDetails}" :disabled="isShowDetails"  style="width:100%" placeholder="户名" @change="line1Change($event,scope.row,scope.$index)">
                    <el-option
          ...
          <el-table-column prop="line2" label="开户行">
              <template slot-scope="scope">
                <el-form-item :prop="'tableData.' + scope.$index + '.line2'" :rules='formData.rules.required'>
                    <el-input :disabled="isShowDetails" size="small" :class="{'isShowDetails':isShowDetails}" v-model="scope.row.line2"  placeholder="开户行"></el-input>
                </el-form-item>
              </template>
          </el-table-column>
          <el-table-column prop="line3" label="账号">
              <template slot-scope="scope">
                <el-form-item :prop="'tableData.' + scope.$index + '.line3'" :rules='formData.rules.required'>
                    <el-input :disabled="isShowDetails" size="small" :class="{'isShowDetails':isShowDetails}"  v-model="scope.row.line3"  placeholder="账号">
                        <el-button slot="append" v-if="!isShowDetails" @click="scFun(scope.$index,scope.row)">收藏</el-button>
                    </el-input>
                </el-form-item>
              </template>
          </el-table-column>
          <el-table-column prop="line4" label="金额（元）">
              <template slot-scope="scope">
                <el-form-item :prop="'tableData.' + scope.$index + '.line4'" >
                    <el-input  size="small" :class="{'isShowDetails':isShowDetails}" :disabled="isShowDetails" v-model="scope.row.line4" :min="0"  label="金额（元）" @input="line4Change($event,scope.row, scope.$index)"  @blur="line4Blur($event,scope.row)" onkeyup="this.value= this.value.match(/\d+(\.\d{0,2})?/) ? this.value.match(/\d+(\.\d{0,2})?/)[0] : ''">
                    </el-input>
                </el-form-item>
              </template>
          </el-table-column>
  ...
  export default {
      ...
      methods:{
          ...
          line1Change (value,row,index) {
            ...
            if (selectData) {
              this.$set(row, 'line2', selectData.openingBank)
              this.$set(row, 'line3', selectData.account)
            } else {
              row.line2 = '';
              row.line3 = '';
            }
          ...
          getTotal () {
            ...
            this.totalLine.totalmoneyUpper = util.convertCurrency(this.totalLine.totalMoney);
          },
          line4Change (e,row,index) {
            this.getTotal();
          },
          getData(){  //保存
                  ...
                  this.$refs.formDom.validate(valid => {
                      if (valid) {
                        let jsonData = {
                          ...
                          totalmoneyUpper:this.totalLine.totalmoneyUpper,
                          tableData:JSON.parse(JSON.stringify(this.formData.tableData))
                        }
                        jsonData.tableData.forEach(item => {
                            let data = this.selectData.find(element => element.id === item.line1);
              ...
              this.gtAccount(row.line3).then(res => {
                if (res) {
                  queryByParentId().then(data=> {
                    ...
                    if (data.length > 0) {
                      let data = this.selectData.find(element => element.id === row.line1);
                      this.bankSelectForm.name = data ? data.name : row.line1;
