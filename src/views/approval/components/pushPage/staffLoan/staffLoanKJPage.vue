<template>
  <d2-container class="loanDetail">
        ...
        <el-card class="box-card">
          <el-form label-width="120px" :model="addModel" ref="formData" :rules="rules">
            ...
            <el-row>
              ...
              <el-col :span="6">
                <el-form-item label="收款金额：" prop="gathermoney">
                  <el-input v-model="addModel.gathermoney"  type="number" placeholder="请输入收款金额">
                    <template slot="append">元</template>
                  </el-input>
                </el-form-item>
              </el-col>
            ...
            <div class="add-customer-content___upload">
                <label for="male">
                    <upload id="male" :isShowUpload="false"  ref="upLoad" :propList="upLoadFileDefaultList"></upload>
                </label>
            </div>
            ...
            <table cellspacing="0" cellpadding="0" border="0" class="payTable">
              ...
              <tbody>
                <tr>
                  <td>1</td>
                  <td>
                    <el-input  v-model="addModel.payname">
                      <el-button
                        slot="append"
                  ...
                  <td>
                    <el-input  v-model="addModel.paybank"></el-input>
                  </td>
                  <td>
                    <el-input  v-model="addModel.payaccount">
                      <el-button slot="append"  @click="collectFun(addModel.payname,addModel.paybank,addModel.payaccount)">收藏</el-button>
                    </el-input>
                  </td>
                  <td>
                    <el-input
                ...
                <tr>
                  <td>2</td>
                  <td>
                    <el-input  v-model="addModel.payname1">
                      <el-button
                        slot="append"
                  ...
                  <td>
                    <el-input  v-model="addModel.paybank1"></el-input>
                  </td>
                  <td>
                    <el-input  v-model="addModel.payaccount1">
                      <el-button slot="append"  @click="collectFun(addModel.payname1,addModel.paybank1,addModel.payaccount1)">收藏</el-button>
                    </el-input>
                  </td>
                  <td>
                    <el-input
          ...
          <el-row>
            <el-col :span="24" style="text-align:center" class="d2-mt-20">
              ...
              >作废</el-button> -->
              <!--  -->
              <!-- <qqt-voucher :data="detailData" :showComfirm="true"></qqt-voucher> -->
              <qqt-voucher ref="voucher"  :voucherShow="voucherShow" @dialogComfirm="dialogComfirm" @dialogCancel="voucherShow=false" :data="detailData" ></qqt-voucher>
              <el-button size="small" type="primary" @click="consentFun(),title = '审批意见', values['flag'] = '1'">同意</el-button>
              <el-button
                size="small"
                type="warning"
                v-if="btnList.indexOf('1') != -1"
    ...
    <el-dialog v-dialogDrag title="银行账号信息" :visible.sync="bankSelectShow" width="50%" top="5vh">
    ...
    <el-dialog
      v-dialogDrag
      title="请选择银行账户类型"
      :visible.sync="bankTypeShow"
      width="30%"
      @close="bankTypeShow = false">
      ...
      <span slot="footer">
        ...
        <el-button @click=" bankTypeShow = false">取 消</el-button>
      </span>
    </el-dialog>
...
import {qqtFlowDiagram,qqtFlowTabs,upload,qqtConsent,qqtVoucher,associatedDocuments} from "../../../../../components/qqt-subassembly";
import {goAdd,getByAccountTypeOther,bankQueryPageList,activitiPage,completeFun,queryById,queryByAccount,collection,queryByParentId} from "../../../../../api/financialManagement/staffLoan";
  ...
  methods: {
    ...
    consentBorrowingApply (item = '', boolean = false) {
        ...
        if (item) {
            ...
            let data = JSON.parse(JSON.stringify(this.addModel));
            if (this.values.flag === '1'){
              jsonData = Object.assign(data, this.values);
            } else {
              jsonData = this.values;
              jsonData.id = data.id;
            }
    ...
    gtAccount (params3) {
      return new Promise((resolve, reject) => {
          queryByAccount ({'account':params3}).then(res => {
            if (res.success) {
               resolve (true)
            } else {
    ...
    // 收藏
    collectFun (params,params2,params3) {
      if (params && params2 && params3) {
        this.gtAccount(params3).then(res => {
          if (res) {
             queryByParentId().then(data=> {
              this.parentList = data;
               if (data.length > 0) {
                this.bankTypeShow = true;
                this.bankSelectForm.name = params;
                this.bankSelectForm.openingBank = params2;
                this.bankSelectForm.account = params3;
               }
             })
          }
        })
      } else {
        this.$message.error('户名,开户行,账号信息不能为空')
      }
    },
    // 转办
    turnToFun() {
      ...
      this.$refs["formData"].validate(valid => {
        if (valid) {
          ...
          if (
            ...
            this.$message.error("借款金额、收款金额和付款金额总和不一致，请确认");
          }
        }
      });
    },
    ...
    payClickFun(name) {
      ...
      this.clickRow = {};
      this.bankName = name;
      this.bankSearch.name = '';
      getByAccountTypeOther(this.bankSearch).then(res => {
        this.tableData = res;
      });
    },
    ...
    pageChange (data) {
      this.bankSearch.pageNo = data.current;
      this.bankSearch.pageSize = data.size;
      bankQueryPageList(this.bankSearch).then(res => {
        this.tableData = res;
      });
    },
    dataSearch() {
