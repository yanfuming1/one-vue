<template>
    <d2-container>
        <template>
            <div class="company-employee-account-header" slot="header">
                <el-form :inline="true" :model="form" ref="form" size="mini">
                    ...
                    <el-form-item>
                        <el-button type="info" @click="handleFormReset">重置
                        </el-button>
                    </el-form-item>
                    <el-form-item>
            ...
            <template>
                <qqt-table :data="employeeAccountsData" :rowHeader="rowHeader" :option="tableOption" @selectionChange="selectChange" @page="handlePaginationChange"></qqt-table>
            </template>
        
        </template>
        <el-dialog v-dialogDrag title="新建账户" v-if="dialogFormVisible" :visible.sync="dialogFormVisible" :data="newAccountData">
            <el-form :model="newAccountData" :rules="rules" ref="form">
                <el-row>
                    <el-col :span="11">
                        <el-form-item label="户名:" prop="name">
                            <el-input v-model="newAccountData.name"></el-input>
                        </el-form-item>
                    </el-col>
                    <el-col :span="11" :offset='2'>
                        <el-form-item label="开户行:" prop="openingBank">
                            <el-input v-model="newAccountData.openingBank" ></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row>
                    <el-col :span="11">
                        <el-form-item label="行号:" prop="onlyCoding">
                            <el-input type="onlyCoding" v-model="newAccountData.onlyCoding"></el-input>
                        </el-form-item>
                    </el-col>
                    <el-col :span="11" :offset='2'>
                        <el-form-item label="银行账号:" prop="account">
                            <el-input type="account" v-model="newAccountData.account" ></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row>
                    ...
                    <el-col :span="11" :offset='2'>
                        <el-form-item label="开户行地址:" prop="address">
                            <el-input v-model="newAccountData.address"></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row>
                    <el-col :span='11'>
                      <el-form-item label="账户类型:" prop="accountType">
                          <el-select v-model="newAccountData.accountType" style='width:100%'>
                              <el-option 
            ...
            <div slot="footer" class="dialog-footer">
                <el-button type="primary" @click="newAccount()">保存</el-button>
        ...
        <el-dialog v-dialogDrag title="编辑账户" :visible.sync="editFormVisible" :data="editAccountData">
            <el-form :model="editAccountData" :rules="rules" ref="editAccountData">
                ...
                <el-row>
                    ...
                    <el-col :span="11" :offset='2'>
                        <el-form-item label="开户行地址:" prop="address">
                            <el-input v-model="editAccountData.address"></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row>
                    <el-col :span='11'>
                      <el-form-item label="账户类型:" prop="accountType">
                          <el-select v-model="editAccountData.accountType" style='width:100%'>
                              <el-option 
                                    v-for="item in options" 
                                    :key="item.value" 
                                    :label="item.label" 
                ...
                <el-row>
                    ...
                    <el-col :span="11" :offset='2'>
                        <el-form-item label="开户行地址:"  >
                            {{ bankAccountDetailsData.address }}
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row>
...
import * as elValudate from "../../bankAccountSettings/components/dataValidation" 
import * as elValudateBank from "../../bankAccountSettings/components/bankNumber" 
import * as elValudateCodings from "../../bankAccountSettings/components/onlyCodings" 

let that = this;
export default {
    name:'bankAccountSettingModule',
    data() {
        return {
            ...
            rules: {
                ...
                phone: [{validator:elValudate.elValidatePhone, required: true, message: "请输入正确的手机号", trigger:  ["blur",'change'] }],
                address: [{ required: true, message: "请输入开户行地址", trigger: ["blur",'change'] }],
                            ...
                            on: {click: () => {
                                ...
                                getCompanyEmployeeAccountById ({'id': params.row.id}).then (res => {
                                    if (res) {
                                        ...
                                        console.log(res)
                                        console.log(this.bankAccountDetailsData)
                                        ...
                                        switch(this.bankAccountDetailsData.accountType) {
                                            case '1': this.bankAccountDetailsData.accountType = '公司员工账户'
                                                break;
                                            case '2': this.bankAccountDetailsData.accountType = '联营人账户'
                                                break;
                                            case '3': this.bankAccountDetailsData.accountType = '公共资源交易中心账户'
                                                break;
                                            case '4': this.bankAccountDetailsData.accountType = '物质设备供应商账户'
                                                break;
                                            case '5': this.bankAccountDetailsData.accountType = '业主付款账户'
                                                break;
                                            case '7': this.bankAccountDetailsData.accountType = '劳务分包账户'
                                                break;
                                            case '8': this.bankAccountDetailsData.accountType = '其他账户'
                                                break;
                                            case '9': this.bankAccountDetailsData.accountType = '公司账户'
                                                break;
                                            default:this.bankAccountDetailsData.accountType = ''
                                            } 
                                    }
                                })
                                } 
    ...
    methods: {
        // 初始化加载数据
        employeeAccounts() {
            console.log(this.bankAccountPathParameters)
            let accountType = ''
            if(this.bankAccountPathParameters) {
                switch (this.bankAccountPathParameters) {
                case '1':
                    accountType = 1
                    break
                case '2':
        ...
        // // 初始化加载数据
        // employeeAccounts(form, isRestore = false, bstatus ,accountType) {
        //     if (isRestore) {
        //       this.page.current = 1;
        //     }
        //     let that = this;
        //     let accountTypeData = this.employeeAccountsData.accountType
    ...
    created () {
        this.bankAccountPathParameters = this.$route.path.substr(val.path.length-1,1)
        console.log(this.bankAccountPathParameters)
    },
    watch: {
        $route (val) {
            this.bankAccountPathParameters = val.path.substr(val.path.length-1,1)
        }
    }
};
</script>
...
<style lang="scss">
    .company-employee-account-header{
        margin-bottom: -25px
    }
</style>
