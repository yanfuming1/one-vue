...
export default {
  ...
  created() {
    ...
    const _url=this.webSurl+"/chatServer?current=1&size="+this.size+"&residentId="+this.userNew.residentId+"&workNum="+this.userNew.workNum+"&fromTo=1";
    ...
    this.ws.onmessage = (evt) => {
      ...
      console.log(evt);
      setTimeout(()=>{
        this.$refs.wrapper.lastChild.scrollIntoView();
      },100)
    };
    // 用于指定报错时的回调函数
...
.chat_box {
  ...
  .wrapper_input_b{
    ...
    span{
      ...
      width: 0%;
      height: 80px;
      // background: url('../../assets/07.png') no-repeat;
