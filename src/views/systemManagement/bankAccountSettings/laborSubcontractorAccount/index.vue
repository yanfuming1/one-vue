...
<!-- 劳务分包商 -->
    <d2-container>
        <template>
            <div class="labor-subcontractor-account-header" slot="header">
        ...
        <el-dialog v-dialogDrag title="新建账户" v-if="dialogFormVisible" :visible.sync="dialogFormVisible" :data="newAccountData">
                ...
                <el-row>
                    <el-col :span='11'>
                        <el-form-item label="账户类型:" prop="accountType">
                            <el-input :disabled="true" placeholder="劳务分包商账户"></el-input>                          
                        </el-form-item>
                    </el-col>
                    <el-col :span="11" :offset='2'>
                        <el-form-item label="账户性质:">
                            <el-select v-model="newAccountData.nature" placeholder="请选择" style='width:100%'>
                                <el-option
                            ...
                            <!-- <el-input v-model="newAccountData.untitled1"></el-input> -->
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row>
           ...
           <!--编辑账户弹框 -->
        <el-dialog v-dialogDrag title="编辑账户" v-if="editFormVisible" :visible.sync="editFormVisible" :data="editAccountData">
            <el-form :model="editAccountData" :rules="rules" ref="editAccountData">
                ...
                <el-row>
                    ...
                    <el-col :span="11" :offset='2'>
                        <el-form-item label="账户性质:">
                            <el-select v-model="editAccountData.nature" placeholder="请选择" style='width:100%'>
                                <el-option
                                v-for="item in options"
                                :key="item.value"
                                :label="item.label"
                ...
                <el-row>
                    <el-col :span="11">
                        <el-form-item label="纳税人识别号:" prop="untitled1">
                            <el-input v-model="editAccountData.untitled1"></el-input>
                        </el-form-item>
                    </el-col>
                </el-row>
            </el-form>    
                ...
                <el-row>
                    <el-col :span="11">
                        <el-form-item label="纳税人识别号:">
                            {{ bankAccountDetailsData.untitled1 }}
                        </el-form-item>
                    </el-col>
                </el-row>
            </el-form>  
...
import * as elValudateTaxpayerCode from "../components/taxpayerCode" 
let that = this;
export default {
    data() {
        return {
            ...
            rules: {
                ...
                untitled1:  [
                    { validator:elValudateTaxpayerCode.taxpayerCode,message: '请输入正确的纳税号', trigger:  ["blur",'change']}
                ]
            },
                            ...
                            on: {click: () => {
                                ...
                                getCompanyEmployeeAccountById ({'id': params.row.id}).then (res => {
                                    if (res) {
                                        ...
                                        switch(this.bankAccountDetailsData.nature) {
                                            case '1': this.bankAccountDetailsData.nature = '自然人'
                                                break
                                            case '2': this.bankAccountDetailsData.nature = '个体工商户'
                                                break
                                            case '3': this.bankAccountDetailsData.nature = '有限公司'
                                                break
                                            case '4': this.bankAccountDetailsData.nature = '其他'
                                                break
                                        }
                                        this.bankAccountDetailsData.accountType = '劳务分包商账户'
                                    }
                                })
                                } 
                            }
    ...
    methods: {
        ...
        newAccount() {
            if (this.newAccountData.nature === '2' || this.newAccountData.nature === '3') {
                if (this.newAccountData.untitled1 === undefined) {
        ...
        editAccount(){
            ...
            if (this.editAccountData.name === '' || this.editAccountData.openingBank === '' || this.editAccountData.account === '' ||
                this.editAccountData.phone === '' || this.editAccountData.accountType === '' || this.editAccountData.nature === '') {
                this.$message.warning("请输入必填内容")
            } else {
                if(temp.nature === '2' || temp.nature === '3') {
                    if (this.editAccountData.untitled1 === undefined || this.editAccountData.untitled1 === '') {
                        this.$message.warning('请输入纳税人识别号')
                    } else {
                        this.editAccountEvent ()
                    }
                } else {
                        this.editAccountEvent ()
                }
...
<style lang="scss">
.labor-subcontractor-account-header{
    margin-bottom: -25px
}
</style>
