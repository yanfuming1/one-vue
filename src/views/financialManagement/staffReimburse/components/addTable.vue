          ...
          <el-table-column prop="line1" label="费用发生时间" width="150px">
              <template slot-scope="scope">
                <el-form-item :prop="'tableData.' + scope.$index + '.line1'" :rules='formData.rules.required'>
                    <el-date-picker
                        :disabled="isShowDetails" size="small" :class="{'isShowDetails':isShowDetails}" v-model="scope.row.line1"  
                        type="date"
                        placeholder="选择日期">
                    </el-date-picker>
                </el-form-item>
            ...
            <el-table-column prop="line4" label="费用名称" width="150px">
                <template slot-scope="scope">
                    <el-form-item :prop="'tableData.' + scope.$index + '.line4'" :rules='formData.rules.required'>
                        <el-select size="small" v-model="scope.row.line4"  :class="{'isShowDetails':isShowDetails}" :disabled="isShowDetails"  style="width:100%"  filterable placeholder="请选择" >
                            <el-option
                            v-for="item in fees"
                            :key="item.id"
                            :label="item.name"
            ...
            <el-table-column prop="line5" label="单位"  width="120px">
              <template slot-scope="scope">
                <el-form-item :prop="'tableData.' + scope.$index + '.line5'" :rules='formData.rules.required'>
                  <el-select size="small" allow-create v-model="scope.row.line5"  :class="{'isShowDetails':isShowDetails}" :disabled="isShowDetails"  style="width:100%"  filterable placeholder="请选择" >
                    <el-option
             ...
             <el-table-column prop="line6" label="数量" min-width="100px">
                <template slot-scope="scope">
                    <el-form-item :prop="'tableData.' + scope.$index + '.line6'" :rules='formData.rules.required'>
                        <el-input  size="small" :class="{'isShowDetails':isShowDetails}" :disabled="isShowDetails" v-model="scope.row.line6" :min="0"  label="数量" @input="line6Change($event,scope.row)"   onkeyup="this.value= this.value.match(/\d+(\.\d{0,2})?/) ? this.value.match(/\d+(\.\d{0,2})?/)[0] : ''">
                        </el-input>
                    </el-form-item>
                </template>
            </el-table-column>
            <el-table-column prop="line7" label="单价（元）" min-width="100px">
                <template slot-scope="scope">
                    <el-form-item :prop="'tableData.' + scope.$index + '.line7'" :rules='formData.rules.validatorMoney'>
                        <el-input  size="small" :class="{'isShowDetails':isShowDetails}" :disabled="isShowDetails" v-model="scope.row.line7" :min="0"  label="单价（元）" @blur="line7Blur($event,scope.row)" @input="line7Change($event,scope.row)" onkeyup="this.value= this.value.match(/\d+(\.\d{0,2})?/) ? this.value.match(/\d+(\.\d{0,2})?/)[0] : ''">
                        </el-input>
                    </el-form-item>
                </template>
            </el-table-column>
            <el-table-column prop="line8" label="合计（元）" width="160px">
                <template slot-scope="scope">
                    <el-form-item :prop="'tableData.' + scope.$index + '.line8'" >
                        <el-input  size="small" :class="{'isShowDetails':isShowDetails}" disabled v-model="scope.row.line8" :min="0"  label="合计（元）" >
                        </el-input>
                    </el-form-item>
                </template>
            </el-table-column>
            <el-table-column prop="line9" label="报销凭证"  width="150px">
              <template slot-scope="scope">
                <el-form-item :prop="'tableData.' + scope.$index + '.line9'" :rules='formData.rules.required'>
                  <el-select size="small" v-model="scope.row.line9"  :class="{'isShowDetails':isShowDetails}" :disabled="isShowDetails"  style="width:100%"  filterable placeholder="请选择" >
                    <el-option v-for="item in dictionaries" :key="item.id" :label="item.name" :value="item.name"></el-option>
                  </el-select>
                </el-form-item>
              </template>
            </el-table-column>
            <el-table-column label="附件" width="150px">
              <template slot-scope="scope">
                <up-load :id="'tableUpload' + scope.$index" :showAddToBtn="showAddToBtn" :showLists='showLists' :isShowUpload="!isShowDetails" @dialogUploadBack="dialogUploadBack" @upLoadFileCallBack="files => upLoadFileCallBack(scope.$index, files)" :propList="scope.row.fileLists" :diffrentRef="'upload' + scope.$index"/>
              </template>
            </el-table-column>
        </el-table>
      </el-form>
        ...
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item label="大写:" style="width:100%">
              <el-input :class="{'isShowDetails':isShowDetails}" disabled  size="small" v-model="totalLine.daxie"  placeholder="请输入"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
export default {
    ...
    methods:{
        ...
        upLoadFileCallBack (index, files) {
          this.formData.tableData[index].fileList = files
        },
        setFile () {
        ...
        addLine(){ //添加行数
            this.$refs.formDom.validate(valid => {
              if (valid) {
                ...
                //添加新的行数
                if (this.formData.tableData.length >=20) {
                  this.$message.error('最大添加限制上限为20条');
                  return
                }
        ...
        line6Change (e,row) {
          if (row.line7) {
            row.line8 = (e * (Number(row.line7) > 0 ? row.line7 : 0)).toFixed(2)
            this.getTotal();
          }
        },
        line7Change (e,row) {
          if (row.line6) {
            row.line8 = (e * (Number(row.line6) > 0 ? row.line6 : 0)).toFixed(2)
            this.getTotal();
          }
        },
        getData(){  //保存
            return new Promise((resolve, reject) => {
                this.$refs.formDom.validate(valid => {
                    if (valid) {
                      let obj = JSON.parse(JSON.stringify(this.formData.tableData));
                      obj.forEach(res => {
                        res.line1 = new Date(res.line1).format('yyyy-MM-dd')
                      })
                      let jsonData = {
