...
import { addEntrepotTree, editEntrepotTree, getEntrepotTreeById } from '../../../../../api/materialEquipmentManagement/basicInformation/entrepot'
...
export default {
    ...
    methods: {
        ...
        saveClassification () {
            this.$refs.generateForm.getData ().then(item => {
                let url = ''
                this.values.id == null ? (url = addEntrepotTree, item['parentId'] = this.treeId ? this.treeId : '0') : url = editEntrepotTree, item['id'] = this.values.id
                url (item).then(res => {
                    if (res.code === 200) {
        ...
        getEntrepotTreeByIdData (id) {
            getEntrepotTreeById ({'id': id}).then (res =>{
    ...
    created () {
        ...
        if (this.treeId != '' && !this.isAdd) {
            this.getEntrepotTreeByIdData (this.treeId)
        }
    },
}
</script>
