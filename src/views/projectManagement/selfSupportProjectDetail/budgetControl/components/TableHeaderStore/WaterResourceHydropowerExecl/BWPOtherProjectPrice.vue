<template>
  <div class="BWP-con-div5">
    <h3 style="text-align:center">其他项目清单计价表</h3>
    ...
    <el-table class="table-div"  header-row-class-name="headerClass" header-cell-class-name="headerCellClass" :data="tableData" border="" :header-cell-style="tableHeaderColor">
      ...
      <el-table-column prop="money" label="金额(元)" align="center">
        <template slot-scope="{row,$index}">
          ...
          <div v-else>
            <span>{{row.money}}</span>
            <i class='el-icon-paperclip' style="font-size:15px;color:#409eff;" v-if="fromChange && row.moneyChange" 
                @click="changeClick(row.budgetId, row.bgId, 'money')"></i>
          </div>
        </template>
      </el-table-column>
...
.BWP-con-div5{
  ...
  .headerClass{
    ...
    th:nth-child(2) { width: 45%!important; }
    th:nth-child(3) { width: 15%!important; }
    th:nth-child(4) { width: 30%!important; }
  }
  .el-table__body{
    .el-table__row{
      td:nth-child(1) { width: 10%!important; }
      td:nth-child(2) { width: 45%!important; }
      td:nth-child(3) { width: 15%!important; }
      td:nth-child(4) { width: 30%!important; }
    }
  }
  .table-div{
