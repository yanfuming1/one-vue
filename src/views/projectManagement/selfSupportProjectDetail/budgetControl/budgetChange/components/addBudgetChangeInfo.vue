<template>
    <d2-container type="fulls" class="addBudgetInfo">
                    ...
                    <template slot="content">
                        ...
                        <fm-generate-form 
                            ...
                            <template slot="excelTable">
                                <pro-category-excel ref="proCategoryExcel" @exportExcel="exportExcel" @changeClick="changeClick" @changeItemAdd="changeItemAdd" :fromChange="fromChange" :values="values" :isShowDetails="transform.isShowDetails"></pro-category-excel>
                            </template>
                        </fm-generate-form>
        ...
        <qqt-send-process v-if="dialogVisible" :dialogVisible="dialogVisible" :processModuleId="processModuleIdProcess" :isSendBack="isSendBack" :prossValue="prossValue" @close="dialogVisible = false" @selectData="updataAddBudgetInfo"/>
        ...
        <change-history-dialog @closeDialog="closeDialog" :data='historyData' :showHistory='showHistory' ></change-history-dialog>
    </d2-container>
</template>
...
import { budgetChangeQueryById, budgetChangeSaveRecord, budgetChangeBgEdit, budgetChangeBgSubmit,
     budgetChangeGetYsChangeRecord, budgetChangeSelectYsChangeRecord, budgetExportXls } from '@/api/selfProject/budgetControl/budgetChange'
import { newFastDFSDel, newFastDFSDownload } from '@/api/system/file/file'
...
export default {
    ...
    data () {
        return {
            ...
            processModuleIdProcess: PORJECT_INFO.ADDBUDGETCHANGEINFO,
    ...
    methods: {
        ...
        // 查看修改历史
        changeClick (changeItem) {
            budgetChangeGetYsChangeRecord(changeItem).then(res => {
        ...
        exportExcel () {
            if (this.values.id) {
                budgetExportXls({"id": this.values.id}).then(res => {
        ...
        changeItemAdd (itemArr) {
            this.changeItemList = itemArr
        },
        ...
        getAddBudgetInfo (id) {
            let url = this.$route.params.changeAdd ? budgetChangeQueryById : getBudgetInfoById
            url ({'id': id}).then (res => {
                if (res) {
                    if (than.$route.params.changeAdd) {
                        delete res.procInstId
                        delete res.applyCode
                    }
                    this.values = Object.assign ({}, res)
                    this.procInstId = res.procInstId || ""
                    this.prossValue.title = `自营预算变更审批:【 ${this.values.budgetName} 】`
                    this.fileDefaultNeaten (res)
                }
            }).finally(() => {
                if (than.$route.params.transform.isShowDetails) {
                    budgetChangeSelectYsChangeRecord({ 'id': this.values.bgId }).then(res => {
                        this.$store.dispatch ('d2admin/budgetChange/selfGetRecordList', res)
                        this.getFormByJson ()
                    })
                } else {
                    this.getFormByJson ()
        ...
        saveAddBudgetInfo () {
            ...
            this.$refs.generateForm.getData().then (data => {
                if (data && item && excelData) {
                    ...
                    budgetChangeBgEdit(this.values).then(res => {
                        if(res) {
                            this.$message.success(res.message)
                            this.closePageTag()
                        }
                    }).finally(() => {
                        if(this.changeItemList.length > 0) {
                            budgetChangeSaveRecord(this.changeItemList).then(res => {
                                // if (res) { console.log("记录添加完成") }
                            })
                        }
                    })
                }
        ...
        submitAddBudgetInfoData () {
            ...
            this.$refs.generateForm.getData ().then (data => {
                if (data && item && excelData) {
                    Object.assign (this.values, data, excelData)
                    if(this.values.excelModelType === ''){
                        this.$message.warning("请导入预算表格!!!")
                        return false
                    }
                    ...
                    this.prossValue = {
                        title: this.$route.params.title || `自营预算变更审批:【 ${data.budgetName} 】`,
        ...
        updataAddBudgetInfo (item, boolean) {
            ...
            boolean ? url = resetSubmitBudgetInfo : url = budgetChangeBgSubmit
        ...
        closePageTag () {
            ...
            let path = ``
            this.stutas ? path = `/approval/${this.stutas}` : path = `/projectManagement/selfSupportProjectDetail/budgetControl/budgetChange`
    ...
    components: {
        ...
        'change-history-dialog': changeHistoryDialog,
