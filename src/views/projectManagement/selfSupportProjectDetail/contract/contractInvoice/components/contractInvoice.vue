<template>
    <d2-container type="fulls">
        ...
        <div class="qqt-container-full___center">
            <qqt-table :data="tableData" ref="table" :exportData="tableData.records" :rowHeader="rowHeader" :dynamicHeader="dynamicHeader" :option="tableOption" @page="handlePaginationChange" />
        </div>
    </d2-container>
</template>
...
<script>
    ...
    export default {
                                ...
                                this.auth.contractType ? h ('el-button', {
                                    ...
                                    on:{
                                        click: () => {
                                            ...
                                            this.routerLink (parms.row.contracttype.includes ('收入合同') ? '销项发票列表' : '进项发票列表', parms.row.id, parms.row.contracttype.includes ('收入合同') ? '1' : '2')
                                        }
                                    }
                                }, parms.row.contracttype.includes ('收入合同') ? '销项发票' : '进项发票') : ''
                            ]
                        }
                    }
                ]
        ...
        methods: {
            ...
            routerLink (title, id, identifying) {
                console.log(title, id, identifying);
                let route = ''
                switch (this.identification) {
                    case '1':
                        route = identifying == '2' ? 'inputSelfProjectInvoice' : 'outputSelfProjectInvoice'
                        break
                    case '2':
                        route = identifying == '2' ? 'inputJointProjectInvoice' : 'outputJointProjectInvoice'
                        break
                    default:
                        route = identifying == '2' ? 'inputInvoice' : 'outputInvoice'
                        break
                }
                this.$router.push ({ path: route, name: route, params: {
                                                                    contractId: id,
                                                                    identifying: identifying,
                                                                    characteristic: this.identification,
                                                                }
                })
            ...
            _validationAuth () {
                switch (this.identification) {
                    case '1':
                        this.auth.contractType =    this.$has ('self:contractInvoice:contractType')
                        break
                    case '2':
                        this.auth.contractType =    this.$has ('self:contractInvoice:contractType')
                        break
                    default:
                        this.auth.contractType = this.$has ('bargain:contractInvoice:contractType')
                        break
                }
            },
        ...
        watch: {
            ...
            $route (val) {
                if (val.path.indexOf('contractInvoice') !== -1) {
                    this.decideLoadDataWay ()
                    this._validationAuth ()
                }
            }
