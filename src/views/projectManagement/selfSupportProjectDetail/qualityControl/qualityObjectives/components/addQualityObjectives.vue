<template>
    <d2-container type="fulls" class="addQualityObjectives">
        <el-card class="box-card closeHeader">
            ...
            <div class="qqt-container-full-content___button">
                <el-button type="warning" v-if="!transform.isShowDetails && procInstId !== ''" @click="isSendBack = true, values['flag'] = '1', submitQualityObjectivesData ()">重新提交</el-button>
                ...
                <el-button type="primary" v-if="!transform.isShowDetails && procInstId === ''" @click="values['flag'] = '1', submitQualityObjectivesData ()">提交</el-button>
                <el-button @click="closePageTag">关闭</el-button>
            </div>
        </el-card>
        <qqt-send-process v-if="dialogVisible" :dialogVisible="dialogVisible" :processModuleId="processModuleId" :isSendBack="isSendBack" :prossValue="prossValue" @close="dialogVisible = false" @selectData="updataQualityObjectives"/>
        <qqt-consent :dialogVisible="visible" :msg="nodeData.msg" :taskId="$route.params.taskId || ''" :type="values['flag']" :isReject="isReject" :isTurnManage="isTurnManage" @close="consentQualityObjectives" />
        <quality-objectives-dialog v-if="qualityObjectives" :dialogVisible="qualityObjectives" :taskData="taskData" @close="resetSubmitUpdataQualityObjectives"></quality-objectives-dialog>
    </d2-container>
</template>

<script>
    import { getQualityObjectivesById, submitQualityObjectives, completeQualityObjectives,
                resetSubmitQualityObjectives, getQualityObjectivesListByIds, getQualityObjectivesList } from '@/api/selfProject/qualityManagement/qualityObjectives'
    ...
    export default {
        ...
        data () {
            return {
                ...
                isTurnManage: false,
                dialogVisible: false,
                qualityObjectives: false,
                processModuleId: PROPRIETARYPROJECT.ADDQQUALITYOBJECTIVES,
                rowHeader: [
                    {
                        prop: 'number',
                        label: '序号',
                                    ...
                                    h ('span', {
                                        ...
                                        [
                                            !this.transform.isShowDetails && this.procInstId !== '' ? h ('el-button', {
                                                ...
                                                on:{
                                                    click: () => {
                                                        this.editQualityObjectives (parms.row)
                                                    }
                                                }
                                            }) : ''
                                        ]
        ...
        methods: {
            ...
            getQualityObjectives (id) {
                getQualityObjectivesListByIds ({ 'ids': id }).then (res => {
                    if (res) {
                        this.values.records = res
                        this.projectInformation.projectCode = res[0]['projectCode']
                        this.prossValue.title = `质量目标审批`
                    }
                })
            },

            getQualityObjectivesList (id) {
                getQualityObjectivesList ({'flowId': id}).then (res => {
                    if (res) {
            ...
            submitQualityObjectivesData () {
                this.$refs.generateForm.getData ().then (data => {
                    if (data) {
                        ...
                        this.prossValue = {
                            title: this.$route.params.title || `质量目标审批`,
            ...
            consentQualityObjectives (item = '', boolean = false) {
                ...
                if (item) {
                    ...
                    this.values['completeTask'] = item
                    completeQualityObjectives (this.values).then (res => {
                        if (res.code === 200) {
                            this.$message.success (res.message)
                            this.closePageTag ()
                        }
            ...
            closePageTag () {
                ...
                let path = ``
                this.stutas ? path = `/approval/${this.stutas}` : path = `/projectManagement/selfSupportProjectDetail/qualityControl/qualityObjectives`
                this.close ({ tagName, path })
            },

            editQualityObjectives (item) {
                this.taskData = { id: item.id, projectCode: this.projectInformation.projectCode }
                this.qualityObjectives = true
            },

            resetSubmitUpdataQualityObjectives (boolean = false) {
                this.qualityObjectives = false
                if (boolean) this.getQualityObjectivesList (this.flowId)
            },
            ...
            updataRouter () {
                ...
                if (this.$route.params && this.$route.params.relevancyId) {
                    this.procInstId == '' ? this.getQualityObjectives (this.$route.params.relevancyId) : (this.flowId = this.$route.params.relevancyId, this.getQualityObjectivesList (this.$route.params.relevancyId))
                }
            }
        },
