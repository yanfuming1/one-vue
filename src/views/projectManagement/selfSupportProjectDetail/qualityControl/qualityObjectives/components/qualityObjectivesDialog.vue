<template>
    <el-dialog title="质量目标" :visible.sync="dialogVisible" width="50%" :before-close="handleClose">
        <fm-generate-form 
            :data="jsonData"
            :remote="remote"
            :value="values"
        ...
        <span slot="footer" class="dialog-footer">
            <el-button v-if="!transform.isShowDetails" type="primary" @click="savaQualityObjectives">确 定</el-button>
...
import { addQualityObjectives, editQualityObjectives, getQualityObjectivesById, getQualityObjectivesPageList } from "@/api/selfProject/qualityManagement/qualityObjectives"
...
export default {
    ...
    data () {
        return {
            processModuleId: PROPRIETARYPROJECT.QUALITYOBJECTIVES,
            remote: {},
            values: {},
            jsonData: {},
            taskTree: [],
    ...
    methods: {
        ...
        getQualityObjectives (id) {
            getQualityObjectivesById ({'id': id}).then (res => {
        ...
        getQualityObjectivesList (projectCode) {
            getQualityObjectivesPageList ({ projectCode }).then (res => {
        ...
        updataRouter () {
            ...
            if (this.taskData) {
                this.taskData.id ? this.getQualityObjectives (this.taskData.id) : ''
                this.taskData.projectCode ? (this.getQualityObjectivesList (this.taskData.projectCode), this.getQualityStandard (this.taskData.projectCode)) : ''
            }
        }
    },
