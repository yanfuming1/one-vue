...
<script>
import { selfRectifyAdd, selfRectifyEdit, selfRectifyQueryById, selfRectifySub, selfRectifyReSub, selfRectifyOperation } from '@/api/selfProject/qualityManagement/qualityImprovement/index'
...
export default {
    ...
    data () {
        return {
            ...
            isTurnManage: false,
            dialogVisible: false,
            code: BASICSCODE.SELF_QUALITY_RECTIFY,
            processModuleId: PROPRIETARYPROJECT.ADDQUALITYIMPROVEMENT,
    ...
    methods: {
        ...
        selectGenerateChange (field, value, model) {
            ...
            switch (field) {
                ...
                case 'divideId':
                    ...
                    if (item) this.values['divideName'] = item.name, this.values.rectifyNumber = item.number
                    break
                case 'constructionTeamId':
                    ...
                    break
                case 'rectifyMethodId':
                    this.values['rectifyMethodId'] = value
                    item = this.rectifyMethodList.find(el => el.id === value)
                    if (item) this.values['rectifyMethod'] = item.name
                    break
            }
        ...
        getAddInspection (id) {
            selfRectifyQueryById ({'id': id}).then (res => {
                if (res) {
                    this.values = Object.assign ({}, res)
                    this.prossValue.title = `自营质量整改审批:【 ${this.values.divideName} 】`
        ...
        saveAddInspection () {
            ...
            this.$refs.generateForm.getData ().then (data => {
                if (data && item) {
                    ...
                    this.values.id == null ? url = selfRectifyAdd : url = selfRectifyEdit
        ...
        submitAddInspectionData () {
            ...
            this.$refs.generateForm.getData ().then (data => {
                if (data && item) {
                    ...
                    this.prossValue = {
                        title: this.$route.params.title || `自营质量整改审批:【 ${this.values.divideName} 】`,
        ...
        updataAddInspection (item, boolean) {
            ...
            boolean ? url = selfRectifyReSub : url = selfRectifySub
            url (this.values).then (res => {
                if (res.code === 200) {
        ...
        consentAddInspection (item = '', boolean = false) {
            ...
            if (item) {
                this.values['completeTask'] = item
                selfRectifyOperation (this.values).then (res => {
                    if (res.code === 200) {
                        this.$message.success (res.message)
                        this.closePageTag ()
                    }
        ...
        getQueryDicByIdList () {
            staffStaffDictionary().then(res => {
                this.staffList = res, this.setRemote ()
            })
            queryDicById ({'id': PROPRIETARYPROJECTDICTIONARY.RECTIFYMETHOD}).then (res => {
                if (res) this.rectifyMethodList = res, this.setRemote ()
            })
        },
        ...
        getQualityObjectivesList (projectCode) {
            getQualityObjectivesPageList ({'projectCode': projectCode}).then (res => {
                if (res) this.qualityObjectives = res, this.setRemote ()
            })
        },
        ...
        setRemote () {
            this.remote = {
                ...
                getRectifyMethod(resolve) { resolve (than.rectifyMethodList) },
        ...
        closePageTag () {
            ...
            let path = ``
            this.stutas ? path = `/approval/${this.stutas}` : path = `/projectManagement/selfSupportProjectDetail/qualityControl/qualityImprovement`
