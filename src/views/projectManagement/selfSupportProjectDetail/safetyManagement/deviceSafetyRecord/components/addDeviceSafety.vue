...
<script>
import { deviceSafetyAdd, deviceSafetyEdit, deviceSafetyQueryById, deviceSafetySubmit, deviceSafetyReSubmit,deviceSafetyOperation } 
    from '@/api/selfProject/safetyManagement/deviceSafety/index'
...
export default {
    ...
    data () {
        return {
            ...
            isTurnManage: false,
            dialogVisible: false,
            processModuleId: PROPRIETARYPROJECT.ADDDEVICESAFETY,
    ...
    methods: {
        ...
        getAddInspection (id) {
            deviceSafetyQueryById ({'id': id}).then (res => {
                if (res) {
                    this.values = Object.assign ({}, res)
                    this.prossValue.title = `自营安全设备巡查审批:【 ${this.values.untitled3} 】`
        ...
        saveAddInspection () {
            ...
            this.$refs.generateForm.getData ().then (data => {
                if (data && boolean && item) {
                    ...
                    this.values.id == null ? url = deviceSafetyAdd : url = deviceSafetyEdit
        ...
        submitAddInspectionData () {
            ...
            this.$refs.generateForm.getData ().then (data => {
                if (data && item) {
                    ...
                    this.prossValue = {
                        title: this.$route.params.title || `自营安全设备巡查审批:【 ${data.untitled3} 】`,
        ...
        updataAddInspection (item, boolean) {
            ...
            boolean ? url = deviceSafetyReSubmit : url = deviceSafetySubmit
            url (this.values).then (res => {
                if (res.code === 200) {
        ...
        consentAddInspection (item = '', boolean = false) {
            ...
            if (item) {
                this.values['completeTask'] = item
                deviceSafetyOperation (this.values).then (res => {
                    if (res.code === 200) {
                        this.$message.success (res.message)
                        this.closePageTag ()
                    }
        ...
        closePageTag () {
            ...
            let path = ``
            this.stutas ? path = `/approval/${this.stutas}` : path = `/projectManagement/selfSupportProjectDetail/safetyManagement/deviceSafetyRecord`
