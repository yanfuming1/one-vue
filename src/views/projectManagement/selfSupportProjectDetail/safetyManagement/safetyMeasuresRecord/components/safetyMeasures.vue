...
<script>
import { selfMeasuresAdd, selfMeasuresEdit, selfMeasuresQueryById, selfMeasuresSubmit, selfMeasuresReSubmit,selfMeasuresOperation } from '@/api/selfProject/safetyManagement/index'
import { selfSafetyQueryById } from '@/api/selfProject/safetyManagement/index'
...
export default {
    ...
    data () {
        return {
            ...
            isTurnManage: false,
            dialogVisible: false,
            processModuleId: PROPRIETARYPROJECT.ADDSAFETYMEASURES,
    ...
    methods: {
        ...
        getAddInspection (id) {
            selfMeasuresQueryById ({'id': id}).then (res => {
                if (res) {
                    this.values = Object.assign ({}, res)
                    this.values.checks = res.check
                    this.prossValue.title = `自营安全整改审批:【 ${this.values.rectificationunit} 】`
                    this.fileDefaultNeaten (res)
                }
            })
        },

        getInspectorIfnfo (id) {
        ...
        saveAddInspection () {
            ...
            this.$refs.generateForm.getData ().then (data => {
                if (data && boolean && item) {
                    ...
                    this.values.checks.forEach( c => {
                        c.identity = 'checks'
                    })
                    this.values.mends.forEach( m => {
                        m.identity = 'mends'
                    })
                    this.values.rectifiExhibits = [...this.values.checks, ...this.values.mends]
                    this.values.id == null ? url = selfMeasuresAdd : url = selfMeasuresEdit
        ...
        submitAddInspectionData () {
            ...
            this.$refs.generateForm.getData ().then (data => {
                if (data && item) {
                    ...
                    this.prossValue = {
                        title: this.$route.params.title || `自营安全整改审批:【 ${data.rectificationunit} 】`,
        ...
        updataAddInspection (item, boolean) {
            ...
            boolean ? url = selfMeasuresReSubmit : url = selfMeasuresSubmit
            url (this.values).then (res => {
                if (res.code === 200) {
        ...
        consentAddInspection (item = '', boolean = false) {
            ...
            if (item) {
                this.values['completeTask'] = item
                selfMeasuresOperation (this.values).then (res => {
                    if (res.code === 200) {
                        this.$message.success (res.message)
                        this.closePageTag ()
                    }
        ...
        closePageTag () {
            ...
            let path = ``
            this.stutas ? path = `/approval/${this.stutas}` : path = `/projectManagement/selfSupportProjectDetail/safetyManagement/safetyMeasuresRecord`
        ...
        updataRouter () {
            ...
            if (this.$route.params && this.$route.params.inspectorId) { 
                this.getInspectorIfnfo (this.$route.params.inspectorId)
            }
        }
    },
