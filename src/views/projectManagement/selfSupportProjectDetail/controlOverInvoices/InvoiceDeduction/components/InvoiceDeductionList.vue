<template>
    <d2-container type="fulls" class="ticketReceipt_registration_list">
        <template slot="header">
            <determine-project v-if="identification == '2'" title='发票抵扣' :proId="projectId"></determine-project>
...
<script>
    import { getInvoiceDeductionList, getSelfInvoiceDeductionList, getJointInvoiceDeductionList,
             checkOrAgains, selfCheckOrAgains, jointCheckOrAgains,
             delByIdInvoiceDeduction, delByIdSelfInvoiceDeduction, delByIdJointInvoiceDeduction,
             delByIdInvoiceDeductionBatch, delByIdSelfInvoiceDeductionBatch, delByIdJointInvoiceDeductionBatch} from '@/api/selfProject/controlOverInvoices/InvoiceDeduction'
    ...
    export default {
                ...
                rowHeader: [
                            ...
                            return h('div', {
                                ...
                                on:{
                                    click: () => {
                                        ...
                                        this.routerLink ('发票抵扣详情', parms.row.id,parms.row.procInstId)
                                    }
                                }
                    ...
                    },
                    {
                        ...
                        render:(h,params) =>{
                            ...
                            }, params.row.deductionThisStr = util.moneySepFormat(params.row.deductionThis)) 
                        }
                    },
                    {
                        prop: 'deductionCumsumStr',
                        label: '累计抵扣金额',
                        width: '120px',
                        isShow: true,
                        render:(h,params) =>{
                            ...
                            }, params.row.deductionCumsumStr = util.moneySepFormat(params.row.deductionCumsum)) 
                        }
                    },
                    {
                        prop: 'createName',
                                ...
                                this.auth.edit ? h ('el-button', {
                                    ...
                                    on:{
                                        click: () => {
                                            ...
                                            this.routerLink ('编辑发票抵扣信息', parms.row.id)
                                        }
                                    }
                                }, '编辑') : '',
        ...
        methods: {
            ...
            getTicketReceiptRegistrationPage (text) {
                ...
                switch (this.identification) {
                    case '1':
                        url = getSelfInvoiceDeductionList
                        break
                    case '2':
                        url = getJointInvoiceDeductionList
                        break
                    default:
                        url = getInvoiceDeductionList
                        break
                }
                url ({
                    ...text,
            ...
            deleteTicketReceiptRegistrationListById (id) {
                util.confirm ().then (boolean => {
                    if (boolean) {
                        ...
                        switch (this.identification) {
                            case '1':
                                url = delByIdSelfInvoiceDeduction
                                break
                            case '2':
                                 url = delByIdJointInvoiceDeduction
                                break
                            default:
                                url = delByIdInvoiceDeduction
                                break
                        }
                        url ({'id': id}).then(res => {
                            if (res.code === 200) {
            ...
            deleteBillingRegistrationListList () {
                ...
                switch (this.identification) {
                    case '1':
                        url = delByIdSelfInvoiceDeductionBatch
                        break
                    case '2':
                        url = delByIdJointInvoiceDeductionBatch
                        break
                    default:
                        url = delByIdInvoiceDeductionBatch
                        break
                }
            ...
            routerLink (title, id = '',procInstId = '') {
                ...
                switch (this.identification) {
                    case '1':
                        route = 'addControlInvoiceDeduction'
                        break
                    case '2':
                        route = 'addJointControlInvoiceDeduction'
                        break
                    default:
                        route = 'addInvoiceDeduction'
                        break
                }
            ...
            _validationAuth () {
                switch (this.identification) {
                    case '1':
                        this.auth.edit       =     this.$has ('invoice:deduction:edit:s')
                        this.auth.delBat     =   this.$has ('invoice:deduction:delete:s')
                        break
                    case '2':
                        this.auth.edit       =     this.$has ('invoice:deduction:edit:j')
                        this.auth.delBat     =   this.$has ('invoice:deduction:delete:j')
                        break
                    default:
                        this.auth.edit       =       this.$has ('invoice:deduction:edit')
                        this.auth.delBat     =     this.$has ('invoice:deduction:delete')
                        break
                }
            },
        ...
        watch: {
            ...
            $route:{
                deep:true,
                handler(val) {
                    if (val.path.indexOf('InvoiceDeduction') !== -1) {
                        this.decideLoadDataWay ()
                        this._validationAuth ()
                    }
                }
