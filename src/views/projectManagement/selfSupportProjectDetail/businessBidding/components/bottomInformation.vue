                ...
                formGetJson ({ 'code': this.processModuleId }).then(res => {
                    if (res) {
                        ...
                        if (!this.isGrid) {
                            ...
                            element.list[0]['name'] = this.fieldTitle + '人'
                            element.list[3]['name'] = this.timeTitle + '时间'

                            if (this.transform.isShowDetails) {
                                element.list[1]['model'] = 'companyName'
                                element.list[2]['model'] = 'departmentName'
                                element.list[1]['type'] = element.list[2]['type'] = 'input'
                            } else {
                                element.list[1]['model'] = 'companyId'
                                element.list[2]['model'] = 'departmentId'
                                element.list[1]['type'] = element.list[2]['type'] = 'select'
                            }
                        }
