            ...
            <template slot="time">
                <el-row class="timeRow">
                    <el-col :span="12">
                        <el-form-item label="计划开工日期" label-width="120px" prop="planStartDate" >
                            <el-date-picker type="date" value-format="yyyy-MM-dd"  v-model="times.planStartDate" :readonly="!isChildrenFlag" @change="timeChange($event,'start')"></el-date-picker>
                        </el-form-item>
                    </el-col>
                    <el-col :span="12">
                        <el-form-item label="计划完工日期" label-width="120px" prop="planEndDate" >
                            <el-date-picker type="date" value-format="yyyy-MM-dd" :readonly="!times.planStartDate || !isChildrenFlag" @change="timeChange2($event,'end')" :picker-options="pickerOptions" @focus="dateFoucus" v-model="times.planEndDate"></el-date-picker>
                        </el-form-item>
                    </el-col>
                </el-row>
            </template>
             <template slot="planLimit" >
                <el-form-item label="计划工期" label-width="120px" class="planLimit">
                     <el-input disabled="" v-model="times.planLimit">
...
export default {
    ...
    data () {
        return {
            pickerOptions: {
                disabledDate(time) {
                    return time.getTime() < new Date(than.times.planStartDate).getTime() - 86400000;
                },
            },
            ruleForm: {
                name: '',
    ...
    methods: {
        ...
        timeChange (value,type) {
            ...
            this.$set(this.times,'planEndDate','')
            this.times.planStartDate = value;
            this.times.planLimit = 0;
        },
        timeChange2 (value) {
            this.times.planEndDate = value;
            // this.times.planLimit = util.dateDiff(value,this.times.planStartDate)
            let data = util.dateDiff(value,this.times.planStartDate) + 1;
            // this.$set(this.times,'planLimit',data)
            this.times.planLimit = data;
            console.log(this.times);
            this.$forceUpdate();
        },
        dateFoucus (value) {
            if (!this.times.planStartDate) {
        ...
        cancel(type) {
            if(type === 'add') {
                if (!this.times.planStartDate && !this.times.planEndDate) {
                ...
                this.$refs.generateForm.getData().then(data => {
                    if (data && item) {
                        ...
                        let obj =Object.assign({}, temData, this.times );
    ...
    watch: {
        planDetail: {
            ...
            handler (val) {
                ...
                this.values = data;
                Object.keys(this.times).forEach(key=>{this.times[key]=data[key]})
                this.$forceUpdate();
                this.getQueryDicByIdList()
