<template>
    <div>
        <el-table
                ...
                :data="tableData"
                style="width: 100%">
            <el-table-column v-if="!isShowDetails" label="" width="50" align="center">
                <template slot-scope="scope" >
                    <i 
                       style="line-height:30px;font-size:20px;color: red;"
...
export default {
    ...
    props: {
        ...
        methods: {
            selectCost(id, row){
                let obj = this.costType.find(res => res.id === id);
                if (obj) {
                    this.tableData[row.$index].type = obj.name;
                }
            },
