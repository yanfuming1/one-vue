            ...
            <div v-if="!fromProcess">
                <div class="cer-borrow">
                    ...
                    <div class="drawer-btn" v-if="!transform.isShowDetails">
                        <el-button type="warning" v-if="useFlow" size="small" @click="subProcess">提交</el-button>
                        <el-button type="success" v-if="!useFlow" size="small" @click="noFlowSave">保存</el-button>
                        <el-button type="info" size="small" v-if="transform.isShowDetails" @click="printTemplate">打印</el-button>
...
import { nodeInfoStartCrew, borrowRecordSubmit, credentialLoanQueryDetailsByIds, borrowRecordOperation, borrowRecordNoFlow} from "@/api/officalDocument/index"
...
export default {
    ...
    data() {
        return {
            ...
            departList: [],
            useFlow: true,
            code: BASICSCODE.ADMIN_CREDENTIAL_LOAN,
            subCompany: ''
        }
    },
    methods: {
        ...
        selectChange(field, value, model){
            switch(field){
                case 'subConpanyId':
                    let num = 0 
                    this.getUserList(value[value.length-1])
                    this.subCompany = '';
...
            })
        ...
        getJson () {
            formGetJson({ 'code': CERTIFICATE.USEFLOWBORROW }).then(res => {
        ...
        noFlowSave () {
            ...
            this.$refs.generateForm.getData().then(res => {
                let temData = this.setData(res)
                temData['registerDate'] = res.register[0]
                temData['registerEndDate'] = res.register[1]
                temData = Object.assign({}, temData, item)
                temData.subCompany = this.subCompany
                borrowRecordNoFlow(temData).then(data => {
        ...
        subProcess () {
            ...
            this.$refs.generateForm.getData().then(res => {
                ...
                this.processModuleId = 'useFlowCerBorrow'
                this.prossValue.title = '证书借用审批'
        ...
        selectDataFun (data) {
            ...
            if(this.isSendBack){
                ...
                borrowRecordOperation(this.subValue).then(res => {
                    this.$message.success("提交成功")
                    this.cancel()
                })
            }else{
                borrowRecordSubmit(this.subValue).then(res => {
                    this.$message.success("提交成功")
                    this.cancel()
                })
            } 
        ...
        consentBorrowingApply(item = '', boolean = false){
            ...
            if (item) {
                ...
                borrowRecordOperation(this.values).then(res => {
                    this.visible = false
        ...
        processGetInfo () {
            ...
            credentialLoanQueryDetailsByIds({ids:this.pushData.processVariables.relevancyId}).then(res => {
                res['register'] = [res.registerDate, res.registerEndDate]
                Object.assign(this.values, res)
            }).finally(() => {
                this.getUserList(this.values.subConpanyId[this.values.subConpanyId.length-1])
                this.getJson()
            })
        },
        getUserList (id) {
    ...
    created () {
        ...
        this.useFlow = this.$route.params.useFlow || false
    },
    mounted() {
        that = this
        ...
        if(pushObj){
            ...
            if(this.$route.params.newSub) {
                this.values.credentialLoanList = this.$route.params.cerInfo
            } else {
                this.fromProcess = true
                this.backlog = false
                this.isSendBack = false
