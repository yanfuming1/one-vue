...
export default {
  ...
  actions: {
      ...
      return new Promise(async resolve => {
        ...
        state.opened = value.map(opened => {
          ...
          if (opened.fullPath === '/index') {
            opened.meta.title = '首页'
            valid.push(1)
            return opened
          }
          // 尝试在所有的支持多标签页的页面里找到 name 匹配的页面
          const find = state.pool.find(item => item.name === opened.name)
          if (opened.meta && find && find.meta) find.meta = opened.meta
          // 记录有效或无效信息
          valid.push(find ? 1 : 0)
          // 返回合并后的数据 新的覆盖旧的
     ...
     * @param {Object} state vuex state
     */
    opend2db ({ state, commit, dispatch }, fullPath) {
      if (state.title && fullPath) {
        let index = state.opened.findIndex (f => f.fullPath === fullPath)
        if (index != -1) {
          state.opened[index]['meta']['title'] = state.title
        }
        commit('titleSet', '')
      }
      return new Promise(async resolve => {
    ...
    openedUpdate ({ state, commit, dispatch }, { index, params, query, fullPath }) {
      return new Promise(async resolve => {
        ...
        // 持久化
        await dispatch('opend2db', fullPath)
        // end
        resolve()
      })
    },
    ...
    add ({ state, commit, dispatch }, { tag, params, query, fullPath }) {
      return new Promise(async resolve => {
        ...
        // 添加进当前显示的页面数组
        state.opened.push(JSON.parse (JSON.stringify (newTag)))
     ...
     */
    close ({ state, commit, dispatch }, { tagName, path = null, name = null, params = {}, query = {} }) {
      // console.log(tagName)
      return new Promise(async resolve => {
        // 下个新的页面
        ...
        if (isCurrent) {
          let routerObj
          if (path == null) {
            if (newPage.name ==="backlog" || newPage.name ==="sendBack") {
              newPage.params = {};
              newPage.query = {};
            }
    ...
    /**
     * @description 保存当前的路由信息
     * @param {Object} state vuex state
     * @param {Object} param 从路由钩子的 to 对象上获取 { name, params, query, fullPath } 路由信息
     */
    routedb ({ state, dispatch }, { name, params, query, fullPath }) {
      return new Promise(async resolve => {
        state.route = {
          name,
          query,
    ...
    /**
     * @description 获取路由信息（页面初始化的获取）
     * @param {*} param0 
     */
    routeLoad ({ state, dispatch }) {
      return new Promise(async resolve => {
        // 获取数据
        state.route = await dispatch('d2admin/db/get', {
          dbName: 'sys',
          path: 'state.route',
          defaultValue: {},
          user: true
